...
# Set your paths
input_csv_path = 'DroneTraining_Capstone_csv-2.csv'
image_directory =  'indu/' #'steve/'
output_directory = 'induannotations/'  #'steveannotations/'
...
#class_dict = {"My Dog": 0}  # You can extend this dictionary as needed
class_dict = {"My Dog": 0, "Not My Dog": 1} #Indu's update
...
with open(input_csv_path, 'r') as csv_file:
    ...
    for row in reader:
        ...
        #if 'My Dog' in region_attributes.values() and region_attributes['name'] in class_dict:
        #    class_id = class_dict[region_attributes['name']]
        #else:
        #    continue

        # Check if the label exists in the class dictionary 
        label_name = region_attributes.get('name', None) #Indu's update
        if label_name in class_dict:#Indu's update
            class_id = class_dict[label_name]#Indu's update
        else:#Indu's update
            continue#Indu's update
